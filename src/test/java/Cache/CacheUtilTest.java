...
public class CacheUtilTest {
    ...
    // 最近最久未使用-LRUCache
    // LRU (least recently used)最近最久未使用缓存。根据使用时间来判定对象是否被持续缓存，当对象被访问时放入缓存，当缓存满了，最久未被使用的对象将被移除。此缓存基于LinkedHashMap，因此当被缓存的对象每被访问一次，这个对象的key就到链表头部。
    ...
    // 举个例子，用户设置key1的超时时间5s，用户在4s的时候调用了get("key1")，此时超时时间重新计算，再过4s调用get("key1")方法值依旧存在。如果想避开这个机制，请调用get("key1", false)方法。
    @Test
    public void test_TimedCache() {
        // 创建缓存，默认4毫秒过期
    ...
    // LFUFileCache、LRUFileCache 这两个用法相似，一个最少使用、一个最久未使用两种文件缓存策略。
    @Test
    public void test_FileCache() {
        // 参数1：容量，能容纳的byte数
